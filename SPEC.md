...
- Web 前端根据 `adminRole` 决定是否显示“后台管理”导航入口；普通用户账号不显示该选项。
...
- 当前仍缺用户搜索筛选参数、复杂报表和真实 AI worker 消费重试任务。
...
- CSV、PDF 和 EXCEL 格式已接入接口和 Web 选择器，当前返回 `status=RESERVED` 与预留提示，后续接入真实文件生成。
...
- 已新增 `AuthService quota integration` 导出用例，覆盖部分导出、打卡证据字段、AI 评分、提醒日志、微信绑定、其他用户数据隔离、密码哈希排除和 PDF 预留格式。
...
- 用户选择性导出和完整导出。
- 已完成当前用户 JSON 部分 / 完整导出闭环；CSV、PDF 和 EXCEL 为接口级预留，尚未生成真实文件。
- 敏感字段加密。
- AI 请求脱敏。
...
- 前端可以自动展示 job 状态。
- 用户可以选择导出部分数据或完整导出。
- 当前第一阶段验收以 `POST /auth/export` JSON 导出为准；CSV、PDF 和 EXCEL 真实文件生成仍是剩余风险。
- 管理后台所有原文查看必须写审计日志。

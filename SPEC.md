...
- 不做冰冷的普通 Todo List。
- 不做纯游戏化，但允许适度成长感和奖励感。
- Web 端视觉与动效升级方向采用 Liquid Glass 风格，基于 `liquid-glass-react` 渐进实现。该库提供 React 玻璃折射、毛玻璃、hover / click、弹性、色散、mouseContainer 和多种 refraction mode 能力，适合作为关键卡片、导航浮层和行动按钮的视觉增强。
- Liquid Glass 只能用于提升层级、反馈和情绪价值，不能牺牲学习工具的可读性、密度和执行效率。
- 管理后台仍保持克制、信息密集和高对比，不使用强折射背景作为主要容器。

Liquid Glass 设计系统约束：

- 依赖策略：前端可引入 `liquid-glass-react`，并封装为本项目内部组件，例如 `GlassPanel`、`GlassButton`、`GlassNav`、`GlassMetricCard`，业务页面不得直接散落第三方组件参数。
- 使用范围：目标驾驶舱顶部摘要、当前目标切换器、计划确认关键决策区、今日任务完成按钮、AI job 状态浮层、健康报告风险卡、奖励愿景板卡片和移动端底部导航可以使用玻璃效果。
- 禁用范围：长文本正文、表格、管理后台数据列表、表单输入主体、导出内容预览和错误详情不使用重折射玻璃效果。
- 兼容策略：Safari 和 Firefox 对 displacement 支持不完整，必须提供 CSS fallback；降级后仍展示半透明背景、边框、高光和清晰文字，不依赖折射才能理解信息。
- 性能预算：玻璃组件首屏数量需要受控；列表项、热力图格子和时间线大量重复节点不逐项启用高成本 shader / displacement。
- 动效策略：默认动画应短、轻、可预测；hover / click 用于确认可交互性，任务完成、奖励达成和救援成功可以有一次性微动效。
- 可访问性：所有玻璃容器需要满足文字对比度，支持 `prefers-reduced-motion`，键盘焦点可见，不把 hover 当作唯一状态提示。
- 主题策略：避免整站单一蓝紫或深色玻璃主题；学习目标页可使用稳定浅色底、低饱和强调色和局部玻璃层级，危险 / 预警 / 稳定状态必须有清晰的语义色。
- 验收方式：前端完成视觉阶段后需要通过 Playwright 截图检查桌面与移动端关键页面，确认无文字重叠、无不可读玻璃背景、hover / click 状态可见、普通用户仍看不到后台管理入口。
...
- 完整 E2E 覆盖注册到目标完成 / 失败复盘、Pro 解锁、提醒日志、导出、后台审计和普通用户后台入口不可见。
- 最终执行 `npm run typecheck`、`npm run test:integration`、`npm run test:e2e`、`npm run build`、`git diff --check`，并同步 `SPEC.md`、`README.md` 与 GitHub `main`。

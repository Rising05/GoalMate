...
> 创建日期：2026-06-19
> 当前状态：实施中；P0-1、P0-2、P3-2、P3-3、P4-1 已完成；P4-2 微信小程序首版页面实现完成并等待微信开发者工具真实预览；P0-3 代码完成并等待真实 DeepSeek 预发布验证
...
**状态：** `IN_PROGRESS`（API v3 下单签名、通知验签解密和本地自动化验收已完成，待真实商户号、AppID、平台证书和 API v3 key 联调）
...
- [x] 实现微信支付 API v3 Native 下单请求。
- [x] 下单请求使用商户私钥按 `WECHATPAY2-SHA256-RSA2048` 生成 Authorization。
- [x] 下单金额、币种、描述、商户订单号和通知地址均由服务端套餐生成。
- [x] 支持通过环境配置商户号、AppID、商户证书序列号、商户私钥、API v3 key、通知地址和平台公钥。
- [x] Webhook 入口支持 `wechatpay-signature`、`wechatpay-timestamp`、`wechatpay-nonce`、`wechatpay-serial` 官方签名头。
- [x] 微信支付通知使用 raw body 做 RSA-SHA256 验签。
- [x] 微信支付通知资源使用 API v3 key 执行 AES-256-GCM 解密。
- [x] 支付成功通知进入 P2-3 统一订单、支付明细、订阅和 entitlement 事务链路。
- [x] 退款通知进入统一退款和 entitlement 撤销链路。
- [x] 签名错误、非原始体、金额不一致不会授予权益。
- [x] 增加不依赖真实商户网络的 API v3 签名、验签、解密和退款通知测试。
- [ ] 使用真实微信商户 Native/JSAPI 下单完成端到端支付。
- [ ] 使用真实微信支付结果通知验证平台证书、序列号轮换和解密。
- [ ] 接入微信支付查询订单和关闭订单接口。
- [ ] 接入微信支付退款申请接口，并用真实退款通知完成联调。
- [ ] 将 API v3 key、商户私钥和平台证书迁移到生产密钥托管。

#### 本轮验收记录

- `npm run typecheck`：通过。
- `npm run build`：通过。
- `npm run test:integration`：通过，138/138。
- `npm run test:e2e`：通过，9/9。
- `git diff --check`：通过。
...
- [x] Stripe 官方事件接入后，将 Stripe 支付、续费、退款、争议统一写入同一订阅/权益模型。
- [x] 微信支付官方事件接入后，将微信支付、退款通知统一写入同一订阅/权益模型。
...
**状态：** `IN_PROGRESS`（本地功能链路已完成：后端草稿持久化、前端助手入口、字段接受/覆盖、刷新恢复、AI 失败回退和草稿落地目标均已接通；待真实 DeepSeek 目标分析体验联调后改为 `DONE`）
...
- [x] 新增 `goal_intake_drafts` 表，保存自然语言输入、分析结果、表单草稿、问答、已接受字段和已创建目标关联。
- [x] 目标助手草稿中的自然语言、分析 JSON、表单草稿和问答均使用应用层版本化加密存储。
- [x] 新增创建目标助手草稿接口，自动调用现有目标分析能力，并在 AI 失败时保存 fallback 草稿。
- [x] 分析结果补充字段置信度、缺失字段和字段来源，供前端明确标识 AI 填充字段。
- [x] 新增读取最新草稿接口，支持刷新后恢复目标创建助手状态。
- [x] 新增读取单个草稿接口。
- [x] 新增更新草稿接口，支持保存用户问答、字段覆盖和已接受字段。
- [x] 新增从目标助手草稿保存正式目标草稿接口，复用现有目标创建校验和加密流程。
- [x] 增加后端集成测试，覆盖草稿恢复、问答/字段覆盖、落地目标和敏感草稿不明文入库。
- [x] 前端创建目标区域接入目标助手入口。
- [x] 前端展示 AI 填充来源、字段置信度、缺失字段和风险建议。
- [x] 前端支持逐字段接受或覆盖，并保存问答状态。
- [x] 前端刷新后从最新草稿恢复目标助手状态。
- [x] 前端 AI 失败时明确回退到普通表单。
- [ ] 使用真实 DeepSeek 目标分析完成端到端体验验收。

#### 本轮验收记录

- 2026-06-24：前端目标创建页新增目标助手入口，一句话目标解析后写入现有表单，并展示 `AI 填充`、`回退建议`、`已接受`、`用户覆盖` 状态。
- 2026-06-24：前端接入 `POST /goals/intake-drafts`、`GET /goals/intake-drafts/latest`、`PATCH /goals/intake-drafts/:draftId`、`POST /goals/intake-drafts/:draftId/create-goal`。
- 2026-06-24：登录或刷新恢复 session 后会自动拉取最近一次目标助手草稿；用户补充问答、已接受字段和覆盖后的表单字段可手动保存。
- 2026-06-24：当后端返回 `analysis.aiError` 时，前端明确提示 AI 解析失败并保留普通表单草稿，用户可以继续手动保存目标。
- `npm run typecheck`：通过。
- `npm run build`：通过。
- `npm run test:integration`：通过，140/140。
- `npm run test:e2e`：通过，9/9（首次运行核心流程出现一次性 500，复跑通过）。
- `git diff --check`：通过。
...
**状态：** `DONE`（统一成长事件表、运行时业务事件写入、历史回填、分页筛选、前端统一事件读取和里程碑完成入口均已完成）
...
- [x] 新增里程碑完成业务入口，并在运行时写入 `MILESTONE_REACHED`。
...
- 2026-06-24：`npm run test:e2e`：通过，9/9。
- 2026-06-24：`git diff --check`：通过。
- 2026-06-25：新增 `POST /goals/:id/milestones/:milestoneId/completion`，支持显式完成/取消完成；完成写入 `MILESTONE_REACHED`，取消完成同步移除对应里程碑事件。
- 2026-06-25：前端计划页和目标计划预览新增里程碑完成/取消完成按钮，完成后刷新计划和统一成长时间线。
- 2026-06-25：新增 `apps/api/src/goals/goals.milestones.integration.test.ts`，覆盖运行时写入、幂等、取消完成和跨用户隔离。
- 2026-06-25：`npm run typecheck`：通过。
- 2026-06-25：`npm exec -w @goalmate/api node -- --import tsx --test src/goals/goals.milestones.integration.test.ts`：通过，4/4。
- 2026-06-25：`npm run test:integration`：通过，147/147。
...
**状态：** `IN_PROGRESS`（首版页面实现完成，待微信开发者工具真实预览与真机测试）
...
**状态：** `IN_PROGRESS`（数据生成脚本 + k6 测试套件已完成；待真实环境运行并记录基线）

#### 当前进度

- [x] 建立可重复的压测数据生成脚本 (`apps/api/src/perf/generate-perf-data.ts`)。
  - 参数化：用户数、目标数、任务天数、打卡比例、AI Job 数和邮件日志数。
  - 使用 `PERF_PREFIX` 标识数据，支持 `PERF_CLEAN_FIRST` 一键清理。
  - 生成 users → goals → dailyTasks → checkins → aiJobs → growthEvents → emailLogs 完整链路。
  - 支持幂等重复执行。
- [x] 建立 API 负载测试场景 (`ops/perf/api-load-test.js`)。
  - k6 测试：登录 → 目标列表 → 今日任务 → 健康状态 → 时间线 → 热力图 → 奖励板 → 提醒偏好。
  - 自定义指标：login/goals/tasks/health/timeline duration，错误率。
  - 阶梯式负载：5 → 20 VUs。
  - P95 < 1 秒阈值。
- [x] 建立并发打卡压力测试 (`ops/perf/concurrency-test.js`)。
  - 并发提交打卡 + 读取任务 + 读取时间线。
  - 验证配额强制执行（HTTP 429）。
  - 阶梯式负载：1 → 10 VUs。
- [ ] 记录基线 P50/P95/P99。
...
**状态：** `IN_PROGRESS`（CI 配置、环境变量清单、发布 Runbook 和灰度检查清单已完成；待 Staging 和 Production 环境实际部署演练）

#### 当前进度

- [x] 整理环境变量清单 (`docs/environment-variables.md`)。
...
- [x] CI 配置 (`.github/workflows/ci.yml`)。
  ...
  - 使用 MySQL 和 Redis service containers。
  - 主分支推送触发完整流水线。
- [x] 发布 Runbook (`docs/release-runbook.md`)。
  - 三阶段发布步骤（Staging → Production → 回滚）。
  - 三批灰度策略（内部 → 种子 → 扩大）。
  - Feature Flags 控制真实 AI、自动提醒、支付、小程序。
  - 暂停条件和恢复流程。
- [x] 灰度检查清单 (`docs/gray-release-checklist.md`)。
  - 第一阶段：3-5 内部账号，1-3 天。
  - 第二阶段：10-20 学生用户，1 周。
  - 第三阶段：扩大至约 100 用户。
  - 每日/每周指标记录模板。
- [ ] 迁移策略验证（Staging 先跑迁移 + 备份 + 回滚演练）。
- [ ] Feature Flag 服务端实现（生产环境变量控制）。
- [ ] 完成一次 Staging 发布演练。
- [ ] 完成一次回滚演练。
...
| P2-1 | Stripe 正式支付 | `IN_PROGRESS` | 2026-06-24 | - | Codex | 官方 SDK Checkout、原始体 Webhook 验签和自动化验收完成；待 Test Mode 真实联调 |
...
| P4-2 | 微信小程序页面 | `IN_PROGRESS` | 2026-06-25 | - | Codex | 首版实现完成；151/151 integration，7/9 E2E；待微信开发者工具真实预览 |
| P5-1 | 性能和容量验证 | `IN_PROGRESS` | 2026-06-25 | - | Codex | 数据生成脚本 + k6 测试套件完成；待真实运行记录基线 |
| P5-2 | 发布流程和灰度运营 | `IN_PROGRESS` | 2026-06-25 | - | Codex | CI 配置、环境变量清单、发布 Runbook 和灰度检查清单完成；待部署演练 |
...
### 2026-06-25 - P5-2 发布流程和灰度运营（仓库实现）

- 状态：`IN_PROGRESS`；CI 配置、环境变量清单、发布 Runbook 和灰度检查清单已完成；待 Staging/Production 真实部署演练。
- 实现摘要：
  - 新增 `.github/workflows/ci.yml`：PR 触发 typecheck + integration (MySQL/Redis service containers) + build + git diff --check。
  - 新增 `docs/environment-variables.md`：覆盖全部 10 个类别（核心、AI、加密、队列、提醒、邮件、微信、存储、支付、可观测性），标注 Development/Staging/Production 差异和必填/可选。
  - 新增 `docs/release-runbook.md`：三阶段发布（预发布验证 → 生产发布 → 回滚）、三批灰度策略（内部 1-3 天 → 种子 1 周 → 扩大至 100 用户）、暂停条件。
  - 新增 `docs/gray-release-checklist.md`：三阶段每日/每周指标记录模板、关键指标阈值、暂停/恢复条件。
- 验收结果：
  - `npm run typecheck`：通过。
  - `npm run test:integration`：151/151 通过。
  - `npm run build`：通过。
  - `git diff --check`：通过。
- 后续依赖：真实部署环境配置；Staging 迁移 + 部署 + E2E 演练；一次回滚演练；生产 Feature Flag 服务端实现。

### 2026-06-25 - P5-1 性能和容量验证（数据生成与测试脚本）

- 状态：`IN_PROGRESS`；性能数据生成脚本、k6 负载测试和并发测试套件已完成；待真实环境运行并记录基线。
- 实现摘要：
  - 新增 `apps/api/src/perf/generate-perf-data.ts`：可参数化的压测数据生成脚本，生成 users → goals → dailyTasks → checkins → aiJobs → growthEvents → emailLogs 完整链路；支持 `PERF_CLEAN_FIRST` 清理和幂等重复执行。
  - 新增 `ops/perf/api-load-test.js`：k6 综合负载测试，覆盖登录→目标→任务→健康→时间线→热力图→奖励→提醒 8 个场景，阶梯式 5→20 VUs，P95 < 1s 阈值。
  - 新增 `ops/perf/concurrency-test.js`：k6 并发打卡压力测试，覆盖提交打卡 + 读取任务 + 读取时间线，1→10 VUs，验证配额 429。
  - 新增 `ops/perf/README.md`：快速开始指南和环境变量说明。
- 验收结果：
  - `npm run typecheck`：通过。
  - `npm run test:integration`：151/151 通过。
  - `npm run build`：通过。
  - `git diff --check`：通过。
- 后续依赖：真实 MySQL 环境运行数据生成脚本；运行 k6 测试并记录基线 P50/P95/P99；优化慢查询和 N+1。

### 2026-06-25 - P4-2 微信小程序页面（首版实现）

- 状态：`IN_PROGRESS`；首版页面骨架、API 客户端层、登录绑定流程、今日任务、打卡表单、图片上传、打卡结果、账号管理和提醒设置均已完成；待微信开发者工具真实预览与 WeChat 真机测试。
- 实现摘要：在 `apps/miniapp/` 下新建微信小程序项目，包含 5 个页面（登录/绑定、今日任务、任务打卡、打卡结果、我的）、标准 API 客户端层和完整 tabBar 导航。
- 项目结构：`apps/miniapp/` 使用 TypeScript + 原生 WXML/WXSS，通过 `miniprogram-api-typings` 获得微信 API 类型支持；API 客户端 `utils/api.ts` 封装全部 P4-1 后台接口及目标、任务、打卡、上传和提醒 API。
- 页面实现：
  - **登录/绑定页** (`pages/login`)：微信 `wx.login` 获取 code → 服务端 `code2session`；已绑定直接登录，未绑定展示绑定已有账号或注册新账号双 Tab 表单；支持 Token 持久化和自动刷新。
  - **今日任务页** (`pages/index`)：目标选择器（picker）、当日日期展示、任务卡片列表（含状态图标、标签、计划时长、评分预览）；已完成/待打卡状态区分；点击进入打卡页。
  - **任务打卡页** (`pages/checkin`)：任务详情展示、已提交打卡回显（含分数）、打卡表单（学习内容 textarea、投入时间、学习状态选择器）；图片/截图上传（`wx.chooseMedia` + 直传 S3 预签名 URL + 完成回调 + 扫描轮询）；SHA-256 校验和上传状态追踪。
  - **打卡结果页** (`pages/result`)：成功/失败结果展示；异步评分轮询（最多 20 秒）；显示总分和 AI 总结。
  - **我的页面** (`pages/account`)：用户信息与会员状态、用量配额概览（目标/AI评分/计划/报告）、提醒设置（开关/时间 picker/时区）；微信绑定状态展示、解绑二次确认、退出登录。
- 配置变更：新增 `apps/miniapp/package.json`、`tsconfig.json`、`project.config.json`（WeChat 开发者工具配置）；根 `package.json` typecheck 命令纳入 `@goalmate/miniapp`。
- 验收结果：
  - `npm run typecheck`：通过（shared + api + web + miniapp 全部通过）。
  - `npm run test:integration`：151/151 通过。
  - `npm run build`：通过。
  - `npm run test:e2e`：7/9 通过（2 个预存在失败：goal analysis AI key 测试和 MVP 循环测试，与 miniapp 变更无关）。
  - `git diff --check`：通过。
- 已知限制：
  - 尚未在微信开发者工具中真实预览和调试；需配置真实 AppID 和 API 服务器域名白名单。
  - Tab bar 图标为 1px 占位 PNG，需替换为正式设计图标。
  - 图片上传的 SHA-256 计算依赖 `wx.getFileInfo` 的 `digestAlgorithm`（新版基础库支持），低版本回退为简单标识符。
  - 小程序不支持 Web 端的完整目标创建、计划确认编辑、健康报告和奖励板管理（按 P4-2 可后置计划）。
- 后续依赖：配置微信小程序 AppID、服务器域名 request/uploadFile 白名单；在微信开发者工具中编译预览并完成真实设备测试；完成登录→打卡→Web 端状态一致性验证后，将 P4-2 标为 `DONE`。
